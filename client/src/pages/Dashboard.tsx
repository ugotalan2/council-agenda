        ...
        {activeTab === 'people' && (
          <div>
                  ...
                  <div className="mb-3">
                    ...
                    {adminOrgs.length === 0 && (
                      <p className="text-muted small">
                        You need to be an admin of at least one organization.
                      </p>
                    )}
                    {adminOrgs.map((org) => (
                      <div key={org.id} className="d-flex align-items-center gap-3 mb-2">
                        <span className="small" style={{ minWidth: 200 }}>
                          {org.name}
                        </span>
                        <select
            ...
            {people.length === 0 && pendingInvites.length === 0 && (
              <div className="alert alert-info">No people yet. Invite someone to get started.</div>
            )}
              ...
              <div key={person.clerkUserId} className="card mb-3">
                <div className="card-body">
                  ...
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    ...
                    {/* Add to org button */}
                    {adminOrgs.filter((o) => !person.organizations.some((po) => po.orgId === o.id))
                      .length > 0 && (
                    ...
                    {person.organizations.map((org, idx) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={async () => {
                            await api.delete(
                              `/api/v1/people/${person.clerkUserId}/organizations/${org.orgId}`
                            )
                            setPeople((prev) =>
                              prev
                                .map((p) =>
                                  p.clerkUserId === person.clerkUserId
                                    ? {
                                        ...p,
                                        organizations: p.organizations.filter(
                                          (o) => o.orgId !== org.orgId
                                        ),
                                      }
                                    : p
